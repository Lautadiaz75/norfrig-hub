      ...
      >
        ...
        >
          ...
          <div className="flex flex-col gap-2 mt-6">
            <button
              onClick={() => fetch(`/api/photo/${photo.id}/open-folder`, { method: 'POST' })}
            ...
            <a
              ...
              className="w-full py-2.5 rounded-xl text-sm font-medium text-center
                         transition-opacity hover:opacity-80 block"
              style={{ background: 'rgba(118,118,128,0.24)', color: 'rgba(235,235,245,0.8)' }}
            >
              Ver original
            </a>
...
function Field({ label, value, mono, small }: { label: string; value: string; mono?: boolean; small?: boolean }) {
  return (
    <div>
      ...
      <p
        className={`leading-snug break-all ${mono ? 'font-mono' : ''} ${small ? 'text-xs' : 'text-sm'}`}
